...
public class GeneratePhpTableModel
{
	...
	public static void main(String[] args) throws IOException
	{
		if (args.length < 2 || args.length > 3) {
			System.out.println(
					"usage: generate-php-table-model <sql dump> [<mapping>] <output directory>");
			System.exit(1);
		}
		...
		GeneratePhpTableModel task = new GeneratePhpTableModel(pathDumpfile,
				pathMapping, dirOutput);
		task.execute();
	}
	...
	public GeneratePhpTableModel(Path pathDumpfile, Path pathMapping,
			Path dirOutput)
	{
	...
	private PhpTableModelGenerator generator = new PhpTableModelGenerator();
